...
public class CollectionControleur {
    ...
    public void setVue(Vue vue) {
        this.vue = vue;
        this.afficherTouteLaCollection();
    }

    private void afficherTouteLaCollection() {
        try {
            if (vue == null || vue.getCollectionPersonnelle() == null) {
                afficherErreur("Impossible d'accéder à la collection.");
                return;
            }

            List<BoiteComposee> boites = vue.getCollectionPersonnelle().getBoites();  

            grilleResultats.getChildren().clear();

            if (boites == null || boites.isEmpty()) {
                afficherErreur("Aucune boîte dans votre collection personnelle.");
                cacherResultats();
                return;
            }

            labelMessage.setStyle("-fx-font-size: 12; -fx-font-weight: bold; -fx-text-fill: #2E7D32;");
            labelMessage.setText(boites.size() + " boîte(s) chargée(s) avec succès.");
            ...
            afficherErreur("Erreur lors du chargement automatique : " + e.getMessage());
            cacherResultats();
            e.printStackTrace();
        }
    }

    private VBox creerCarte(BoiteComposee boite){
        VBox carte = new VBox();
        carte.setSpacing(0);
